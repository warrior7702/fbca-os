...
Deno.serve(async (req) => {
    try {
        ...
        const categorizedResponse = await fetch(
            "https://graph.microsoft.com/v1.0/me/mailFolders('Inbox')/messages?$filter=categories/any()&$top=100&$select=id,subject,from,receivedDateTime,isRead,hasAttachments,importance,categories,webLink,internetMessageId,bodyPreview&$orderby=receivedDateTime desc",
            {
                headers: {
