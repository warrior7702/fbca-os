                ...
                {routineTasks.length > 0 ? (
                  <div className="space-y-3">
                    {routineTasks.map((task) => (
                      <div
                        key={task.id}
                        className="p-4 bg-white rounded-xl border-l-4 border-l-indigo-500 shadow-sm hover:shadow-md transition-all flex items-center justify-between"
                      >
                        <div className="flex items-center gap-4">
                          <RepeatIcon className="w-5 h-5 text-indigo-500" />
                          {/* Assignee Avatar */}
                          ...
                          <div className="flex-1">
                            ...
                            <div className="flex items-center gap-2 mt-1 flex-wrap">
                              ...
                              <span className="text-xs text-slate-600">{task.assigneeName || 'Unassigned'}</span>
                            </div>
                          </div>
                        </div>
                        <div className="flex items-center gap-2">
                          <Select
                            ...
                            onValueChange={(newAssignee) => {
                              const worker = departmentWorkers.find(w => w.user_email === newAssignee);
                              const updatedRoutineTasks = routineTasks.map(t => 
                                t.id === task.id 
                                  ? {...t, assignee: newAssignee, assigneeName: worker?.user_name || newAssignee}
                                  : t
                              );
                              setRoutineTasks(updatedRoutineTasks);
                              toast.success('Routine task reassigned');
                            }}
                          >
                  ...
                  <div className="text-center py-6">
                    <RepeatIcon className="w-10 h-10 text-indigo-200 mx-auto mb-2" />
                    <p className="text-sm text-slate-500">No routine tasks configured</p>
                    <p className="text-xs text-slate-400">Add routine tasks in Department Info tab</p>
                  </div>
                )}
              </CardContent>
            </Card>
