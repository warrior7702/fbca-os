...
const defaultApps = [
  ...
  { id: "myapprovals", name: "Approvals", icon: ClipboardCheck, color: "from-orange-500 to-red-500", path: "MyApprovals", hasNotifications: true },
...
export default function Dashboard() {
  ...
  const loadPendingApprovals = async () => {
    try {
      ...
      setPendingApprovalsCount(response.data.count || 0);
    } catch (error) {
                        ...
                        app.path.startsWith("mailto:") ? (
                          ...
                          <Link to={createPageUrl(app.path)} className="block h-full">
                            ...
                            >
                              <div className={`w-16 h-16 bg-gradient-to-br ${app.color} rounded-2xl shadow-2xl flex items-center justify-center relative`}>
                                <app.icon className="w-8 h-8 text-white" />
                                {app.hasNotifications && pendingApprovalsCount > 0 && (
                                  <Badge className="absolute -top-2 -right-2 bg-red-500 text-white h-6 min-w-6 flex items-center justify-center rounded-full border-2 border-white shadow-lg">
