...
export default function MyApprovals() {
  ...
  const [searchOpenFor, setSearchOpenFor] = useState(null);
  ...
  const handleSendCodeToPCO = (approval) => {
    setSearchOpenFor(approval.request_id);
    setSearchQuery("");
    setSearchResults([]);
  };

  const handleCardholderSelected = async (cardholder, approval) => {
    try {
          ...
          <AnimatePresence>
            {approvals.length === 0 ? (
              ...
              approvals.map((approval) => {
                ...
                const isSearchOpen = searchOpenFor === approval.request_id;
                      ...
                      <CardContent className="space-y-4">
                        ...
                        <AnimatePresence>
                          {isSearchOpen && (
                            ...
                            >
                              <div className="p-4 bg-white rounded-lg border-2 border-blue-300 space-y-3">
                                <div className="flex items-center justify-between mb-2">
                                  <div className="flex items-center gap-2">
                                    <Key className="w-5 h-5 text-blue-600" />
                                    <h3 className="font-semibold text-slate-900">Search Door Codes</h3>
                                  </div>
                                  <Button
                                    variant="ghost"
                                    size="icon"
                                ...
                                <div className="max-h-64 overflow-y-auto">
                                  ...
                                  {searchQuery.length >= 2 && !searching && searchResults.length === 0 && (
                                    <div className="text-center py-6 text-slate-500">
                                      <p className="text-sm">No cardholders found for "{searchQuery}"</p>
                                    </div>
                                  )}

                                  {searchResults.length > 0 && (
                                    <div className="space-y-2">
                                      {searchResults.map((cardholder) => (
                                        <motion.button
                                          key={cardholder.id}
                                          initial={{ opacity: 0, y: 10 }}
                                          animate={{ opacity: 1, y: 0 }}
                                          onClick={() => handleCardholderSelected(cardholder, approval)}
                                          className="w-full flex items-center gap-3 p-3 hover:bg-blue-50 rounded-lg transition-colors text-left border border-slate-200 hover:border-blue-300"
                                        >
